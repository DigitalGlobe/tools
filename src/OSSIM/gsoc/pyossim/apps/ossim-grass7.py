...
def OssimToGrass(filename):
    ...
    for band in xrange(ossimdata.shape[0]):
        ...
        gdata = pygrass.raster.RasterNumpy(grass_band)
...
def GrassToOssim(mapname):
    registry = ossimImageHandlerRegistry.instance()
    # read map
    gdata = pygrass.raster.RasterNumpy(mapname)
    
    memSource = ossimMemoryImageSource()
    stype = PYOSSIM_FLOAT32
    imdata = ossimImageData(memSource,stype,1)
    memSource.initialize()
    WriteArrayToImageData(imdata,gdata,0)
    outfile = mapname +".jpg"
    WriteImageDataToFile(imdata,outfile)

def print_usage():

    print "Usage: python ossim-grass7.py <image1> <option>"
    print "Options are '-tograss' or '-tossim'. default is read."
    print "Example: ossim-grass7.py <grass-raster-name> -toossim"
    print "Example: ossim-grass7.py <image-file-name> -tograss"
    sys.exit(0)
