...
if [ $# -ne 2 ]
then
  echo "Usage: `basename $0` <path_to_dev_root> <install_prefix>"
  exit 1
fi
...
OS=linux
CONFIG_DIR=$OSSIM_DEV_HOME/scripts/config_scripts/$OS
THREADS=4
...
CMAKE_BASE=cmake-2.8.12.1
CMAKE_URL=http://www.cmake.org/files/v2.8/$CMAKE_BASE.tar.gz
...
read answer
if [ "$answer" == 'y' ]; then
   command="sudo yum install aspell automake cmake cvs expat-devel freeglut-devel freetype-devel gcc-c++ gcc-gfortran git libcurl-devel libjpeg-turbo-devel libxml2-devel minizip-devel openssl openssl-devel qt-devel subversion xemacs xemacs-common xemacs-info xemacs-muse xemacs-packages-base xemacs-packages-extra yasm zlib-devel"
   echo $command
   $command
fi
