...
int main(int argc, char **argv)
{
    ...
    (void)htThreadCreate(mainServerLoop, (void *)&serversock, HT_FALSE);
    /*now enter the client loop */
    tid = htThreadCreate(mainClientLoop, NULL, HT_TRUE);
    (void)htThreadJoin(tid, (void **)&exitcode);
    printf("mainClientLoop exited with code %d\n", exitcode);
    shutdown = NL_TRUE;
    sleep(4);
    nlShutdown();
	(void)htMutexDestroy(&printmutex);
    return 0;
}
