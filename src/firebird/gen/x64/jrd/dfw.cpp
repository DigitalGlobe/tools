...
static const UCHAR	jrd_344 [114] =
   {	// blr string 
...
static const UCHAR	jrd_356 [163] =
   {	// blr string 

4,2,4,3,1,0,7,0,4,2,2,0,7,0,7,0,4,1,4,0,41,3,0,32,0,7,0,7,0,7,
0,4,0,1,0,41,3,0,32,0,12,0,2,7,'C',2,'K',4,0,0,'K',6,0,1,'G',
58,47,24,1,8,0,24,0,1,0,58,47,24,0,0,0,25,0,0,0,61,24,1,0,0,255,
2,14,1,2,1,24,0,1,0,25,1,0,0,1,21,8,0,1,0,0,0,25,1,1,0,1,24,0,
2,0,41,1,3,0,2,0,255,17,0,9,13,12,3,18,0,12,2,10,0,2,2,1,41,2,
1,0,0,0,24,2,2,0,255,255,255,14,1,1,21,8,0,0,0,0,0,25,1,1,0,255,
255,76
   };	// end of blr string 
static const UCHAR	jrd_369 [114] =
   {	// blr string 
...
static const UCHAR	jrd_377 [129] =
   {	// blr string 
...
static const UCHAR	jrd_386 [133] =
   {	// blr string 
...
static const UCHAR	jrd_394 [211] =
   {	// blr string 
...
static const UCHAR	jrd_410 [149] =
   {	// blr string 
...
static const UCHAR	jrd_422 [152] =
   {	// blr string 
...
static const UCHAR	jrd_435 [112] =
   {	// blr string 
...
static const UCHAR	jrd_442 [193] =
   {	// blr string 
...
static const UCHAR	jrd_458 [141] =
   {	// blr string 
...
static const UCHAR	jrd_470 [148] =
   {	// blr string 
...
void DFW_update_index(const TEXT* name, USHORT id, const SelectivityList& selectivity, jrd_tra* transaction)
{
   ...
   struct {
          double  jrd_466;	// RDB$STATISTICS 
          SSHORT jrd_467;	// RDB$INDEX_ID 
   } jrd_465;
   struct {
          double  jrd_462;	// RDB$STATISTICS 
          SSHORT jrd_463;	// gds__utility 
          SSHORT jrd_464;	// RDB$INDEX_ID 
   } jrd_461;
   struct {
          TEXT  jrd_460 [32];	// RDB$INDEX_NAME 
   } jrd_459;
   struct {
          SSHORT jrd_481;	// gds__utility 
   } jrd_480;
   struct {
          double  jrd_478;	// RDB$STATISTICS 
          SSHORT jrd_479;	// RDB$FIELD_POSITION 
   } jrd_477;
   struct {
          double  jrd_474;	// RDB$STATISTICS 
          SSHORT jrd_475;	// gds__utility 
          SSHORT jrd_476;	// RDB$FIELD_POSITION 
   } jrd_473;
   struct {
          TEXT  jrd_472 [32];	// RDB$INDEX_NAME 
   } jrd_471;
/**************************************
 *
 ...
 **************************************/
	...
	if (dbb->dbb_ods_version >= ODS_VERSION11) {
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_470, sizeof(jrd_470), true);
		gds__vtov ((const char*) name, (char*) jrd_471.jrd_472, 32);
		EXE_start (tdbb, request, transaction);
		EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_471);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 12, (UCHAR*) &jrd_473);
		   if (!jrd_473.jrd_475) break;
			...
			/*MODIFY SEG USING*/
			{
				...
				/*SEG.RDB$STATISTICS*/
				jrd_473.jrd_474 = selectivity[/*SEG.RDB$FIELD_POSITION*/
	       jrd_473.jrd_476];
			/*END_MODIFY;*/
			jrd_477.jrd_478 = jrd_473.jrd_474;
			jrd_477.jrd_479 = jrd_473.jrd_476;
			EXE_send (tdbb, request, 2, 10, (UCHAR*) &jrd_477);
			}
		/*END_FOR;*/
		   EXE_send (tdbb, request, 3, 2, (UCHAR*) &jrd_480);
		   }
		}
	...
	gds__vtov ((const char*) name, (char*) jrd_459.jrd_460, 32);
	EXE_start (tdbb, request, transaction);
	EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_459);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 12, (UCHAR*) &jrd_461);
	   if (!jrd_461.jrd_463) break;
		...
		/*MODIFY IDX USING*/
		{
			...
			/*IDX.RDB$INDEX_ID*/
			jrd_461.jrd_464 = id + 1;
			/*IDX.RDB$STATISTICS*/
			jrd_461.jrd_462 = selectivity.back();
		/*END_MODIFY;*/
		jrd_465.jrd_466 = jrd_461.jrd_462;
		jrd_465.jrd_467 = jrd_461.jrd_464;
		EXE_send (tdbb, request, 2, 10, (UCHAR*) &jrd_465);
		}
	/*END_FOR;*/
	   EXE_send (tdbb, request, 3, 2, (UCHAR*) &jrd_468);
	   }
	}
...
{
   struct {
          SSHORT jrd_434;	// gds__utility 
   } jrd_433;
   struct {
          SLONG  jrd_431;	// RDB$FILE_LENGTH 
          SLONG  jrd_432;	// RDB$FILE_START 
   } jrd_430;
   struct {
          SLONG  jrd_427;	// RDB$FILE_START 
          SLONG  jrd_428;	// RDB$FILE_LENGTH 
          SSHORT jrd_429;	// gds__utility 
   } jrd_426;
   struct {
          SSHORT jrd_424;	// RDB$SHADOW_NUMBER 
          SSHORT jrd_425;	// RDB$FILE_SEQUENCE 
   } jrd_423;
   struct {
          SLONG  jrd_439;	// RDB$FILE_LENGTH 
          SLONG  jrd_440;	// RDB$FILE_START 
          SSHORT jrd_441;	// gds__utility 
   } jrd_438;
   struct {
          SSHORT jrd_437;	// RDB$SHADOW_NUMBER 
   } jrd_436;
   struct {
          TEXT  jrd_457 [256];	// RDB$FILE_NAME 
   } jrd_456;
   struct {
          SSHORT jrd_455;	// gds__utility 
   } jrd_454;
   struct {
          SLONG  jrd_452;	// RDB$FILE_START 
          SSHORT jrd_453;	// RDB$FILE_SEQUENCE 
   } jrd_451;
   struct {
          TEXT  jrd_446 [256];	// RDB$FILE_NAME 
          SLONG  jrd_447;	// RDB$FILE_START 
          SSHORT jrd_448;	// gds__utility 
          SSHORT jrd_449;	// RDB$FILE_SEQUENCE 
          SSHORT jrd_450;	// RDB$SHADOW_NUMBER 
   } jrd_445;
   struct {
          TEXT  jrd_444 [256];	// RDB$FILE_NAME 
   } jrd_443;
/**************************************
 *
 *	a d d _ f i l e
 ...
 **************************************/
	...
	case 4:
		...
		{
		if (!handle)
		   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_442, sizeof(jrd_442), true);
		gds__vtov ((const char*) work->dfw_name.c_str(), (char*) jrd_443.jrd_444, 256);
		EXE_start (tdbb, handle, transaction);
		EXE_send (tdbb, handle, 0, 256, (UCHAR*) &jrd_443);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 266, (UCHAR*) &jrd_445);
		   if (!jrd_445.jrd_448) break;
			...
			if (work->dfw_type != dfw_add_shadow)
			{
				/*MODIFY X USING*/
				{
					...
					ISC_expand_filename(/*X.RDB$FILE_NAME*/
							    jrd_445.jrd_446, 0,
						/*X.RDB$FILE_NAME*/
						jrd_445.jrd_446, sizeof(/*X.RDB$FILE_NAME*/
	 jrd_445.jrd_446), false);
				/*END_MODIFY;*/
				gds__vtov((const char*) jrd_445.jrd_446, (char*) jrd_456.jrd_457, 256);
				EXE_send (tdbb, handle, 4, 256, (UCHAR*) &jrd_456);
				}
			}
				     ...
				     jrd_445.jrd_447) < max)
			{
				...
				{
				if (!handle2)
				   handle2 = CMP_compile2 (tdbb, (UCHAR*) jrd_435, sizeof(jrd_435), true);
				jrd_436.jrd_437 = jrd_445.jrd_450;
				EXE_start (tdbb, handle2, transaction);
				EXE_send (tdbb, handle2, 0, 2, (UCHAR*) &jrd_436);
				while (1)
				   {
				   EXE_receive (tdbb, handle2, 1, 10, (UCHAR*) &jrd_438);
				   if (!jrd_438.jrd_441) break;
						start = /*Y.RDB$FILE_START*/
							jrd_438.jrd_440 + /*Y.RDB$FILE_LENGTH*/
   jrd_438.jrd_439;
				/*END_FOR;*/
				   }
				}
			...
			if ((shadow_number &&
				(section = SDW_add_file(tdbb, /*X.RDB$FILE_NAME*/
							      jrd_445.jrd_446, start, shadow_number))) ||
				(section = PAG_add_file(tdbb, /*X.RDB$FILE_NAME*/
							      jrd_445.jrd_446, start)))
			{
				/*MODIFY X USING*/
				{
				
					/*X.RDB$FILE_SEQUENCE*/
					jrd_445.jrd_449 = section;
					/*X.RDB$FILE_START*/
					jrd_445.jrd_447 = start;
				/*END_MODIFY;*/
				jrd_451.jrd_452 = jrd_445.jrd_447;
				jrd_451.jrd_453 = jrd_445.jrd_449;
				EXE_send (tdbb, handle, 2, 6, (UCHAR*) &jrd_451);
				}
			}
		/*END_FOR;*/
		   EXE_send (tdbb, handle, 3, 2, (UCHAR*) &jrd_454);
		   }
		}
		...
		if (section)
		{
			...
			{
			if (!handle)
			   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_422, sizeof(jrd_422), true);
			jrd_423.jrd_424 = shadow_number;
			jrd_423.jrd_425 = section;
			EXE_start (tdbb, handle, transaction);
			EXE_send (tdbb, handle, 0, 4, (UCHAR*) &jrd_423);
			while (1)
			   {
			   EXE_receive (tdbb, handle, 1, 10, (UCHAR*) &jrd_426);
			   if (!jrd_426.jrd_429) break;
				/*MODIFY X USING*/
				{
				
					/*X.RDB$FILE_LENGTH*/
					jrd_426.jrd_428 = start - /*X.RDB$FILE_START*/
	   jrd_426.jrd_427;
				/*END_MODIFY;*/
				jrd_430.jrd_431 = jrd_426.jrd_428;
				jrd_430.jrd_432 = jrd_426.jrd_427;
				EXE_send (tdbb, handle, 2, 8, (UCHAR*) &jrd_430);
				}
			/*END_FOR;*/
			   EXE_send (tdbb, handle, 3, 2, (UCHAR*) &jrd_433);
			   }
			}
			CMP_release(tdbb, handle);
		}
...
static bool add_shadow( thread_db* tdbb, SSHORT phase, DeferredWork* work, jrd_tra* transaction)
{
   ...
   struct {
          ...
          SSHORT jrd_406;	// RDB$FILE_FLAGS 
          SSHORT jrd_407;	// RDB$FILE_SEQUENCE 
   } jrd_404;
   struct {
          TEXT  jrd_398 [256];	// RDB$FILE_NAME 
          SLONG  jrd_399;	// RDB$FILE_START 
          SSHORT jrd_400;	// gds__utility 
          SSHORT jrd_401;	// RDB$FILE_SEQUENCE 
          SSHORT jrd_402;	// RDB$FILE_FLAGS 
          SSHORT jrd_403;	// RDB$SHADOW_NUMBER 
   } jrd_397;
   struct {
          TEXT  jrd_396 [256];	// RDB$FILE_NAME 
   } jrd_395;
   struct {
          SSHORT jrd_421;	// gds__utility 
   } jrd_420;
   struct {
          TEXT  jrd_419 [256];	// RDB$FILE_NAME 
   } jrd_418;
   struct {
          TEXT  jrd_414 [256];	// RDB$FILE_NAME 
          SSHORT jrd_415;	// gds__utility 
          SSHORT jrd_416;	// RDB$FILE_FLAGS 
          SSHORT jrd_417;	// RDB$SHADOW_NUMBER 
   } jrd_413;
   struct {
          TEXT  jrd_412 [256];	// RDB$FILE_NAME 
   } jrd_411;
/**************************************
 *
 *	a d d _ s h a d o w
 ...
 **************************************/
	...
	case 4:
		...
		{
		if (!handle)
		   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_410, sizeof(jrd_410), true);
		gds__vtov ((const char*) work->dfw_name.c_str(), (char*) jrd_411.jrd_412, 256);
		EXE_start (tdbb, handle, transaction);
		EXE_send (tdbb, handle, 0, 256, (UCHAR*) &jrd_411);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 262, (UCHAR*) &jrd_413);
		   if (!jrd_413.jrd_415) break;
			...
			/*MODIFY F USING*/
			{
				...
				expanded_fname.copyTo(/*F.RDB$FILE_NAME*/
						      jrd_413.jrd_414, sizeof(/*F.RDB$FILE_NAME*/
	 jrd_413.jrd_414));
			/*END_MODIFY;*/
			gds__vtov((const char*) jrd_413.jrd_414, (char*) jrd_418.jrd_419, 256);
			EXE_send (tdbb, handle, 2, 256, (UCHAR*) &jrd_418);
			}

			for (shadow = dbb->dbb_shadow; shadow; shadow = shadow->sdw_next)
			{
				if ((/*F.RDB$SHADOW_NUMBER*/
				     jrd_413.jrd_417 == shadow->sdw_number) && !(shadow->sdw_flags & SDW_IGNORE))
				{
					if (/*F.RDB$FILE_FLAGS*/
					    jrd_413.jrd_416 & FILE_shadow)
					{
		...
		/*END_FOR;*/
		   EXE_send (tdbb, handle, 3, 2, (UCHAR*) &jrd_420);
		   }
		}
		CMP_release(tdbb, handle);
		...
		{
		if (!handle)
		   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_394, sizeof(jrd_394), true);
		gds__vtov ((const char*) expanded_fname.c_str(), (char*) jrd_395.jrd_396, 256);
		EXE_start (tdbb, handle, transaction);
		EXE_send (tdbb, handle, 0, 256, (UCHAR*) &jrd_395);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 268, (UCHAR*) &jrd_397);
		   if (!jrd_397.jrd_400) break;
			...
			if (!sequence)
				SDW_add(tdbb, /*Y.RDB$FILE_NAME*/
					      jrd_397.jrd_398, /*Y.RDB$SHADOW_NUMBER*/
  jrd_397.jrd_403, /*Y.RDB$FILE_FLAGS*/
  jrd_397.jrd_402);
				 ...
				 jrd_397.jrd_399)
			{
				if (!shadow)
				{
					for (shadow = dbb->dbb_shadow; shadow; shadow = shadow->sdw_next)
					{
						if ((/*Y.RDB$SHADOW_NUMBER*/
						     jrd_397.jrd_403 == shadow->sdw_number) &&
				...
				add_sequence = SDW_add_file(tdbb, /*Y.RDB$FILE_NAME*/
								  jrd_397.jrd_398, min_page, /*Y.RDB$SHADOW_NUMBER*/
	    jrd_397.jrd_403);
			}
			...
			if (!sequence || (/*Y.RDB$FILE_START*/
					  jrd_397.jrd_399 && add_sequence))
			{
				/*MODIFY Y*/
				{
				
					/*Y.RDB$FILE_FLAGS*/
					jrd_397.jrd_402 |= FILE_shadow;
					/*Y.RDB$FILE_SEQUENCE*/
					jrd_397.jrd_401 = sequence;
					/*Y.RDB$FILE_START*/
					jrd_397.jrd_399 = min_page;
				/*END_MODIFY;*/
				jrd_404.jrd_405 = jrd_397.jrd_399;
				jrd_404.jrd_406 = jrd_397.jrd_402;
				jrd_404.jrd_407 = jrd_397.jrd_401;
				EXE_send (tdbb, handle, 2, 8, (UCHAR*) &jrd_404);
				}
				sequence++;
			}

		/*END_FOR;*/
		   EXE_send (tdbb, handle, 3, 2, (UCHAR*) &jrd_408);
		   }
		}
		CMP_release(tdbb, handle);
		break;
...
{
   struct {
          TEXT  jrd_392 [32];	// RDB$FIELD_SOURCE 
          SSHORT jrd_393;	// gds__utility 
   } jrd_391;
   struct {
          TEXT  jrd_388 [32];	// RDB$DEPENDENT_NAME 
          SSHORT jrd_389;	// RDB$DEPENDED_ON_TYPE 
          SSHORT jrd_390;	// RDB$DEPENDENT_TYPE 
   } jrd_387;
/**************************************
 *
 ...
 **************************************/
	...
	for (size_t pos = 0; !err && pos < names.getCount(); ++pos)
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_386, sizeof(jrd_386), true);
		gds__vtov ((const char*) names[pos].c_str(), (char*) jrd_387.jrd_388, 32);
		jrd_387.jrd_389 = obj_relation;
		jrd_387.jrd_390 = obj_computed;
		EXE_start (tdbb, request, transaction);
		EXE_send (tdbb, request, 0, 36, (UCHAR*) &jrd_387);
		while (1)
		   {
...
{
   struct {
          TEXT  jrd_374 [32];	// RDB$DEPENDENT_NAME 
          SSHORT jrd_375;	// gds__utility 
          SSHORT jrd_376;	// RDB$DEPENDENT_TYPE 
   } jrd_373;
   struct {
          TEXT  jrd_371 [32];	// RDB$DEPENDED_ON_NAME 
          SSHORT jrd_372;	// RDB$DEPENDED_ON_TYPE 
   } jrd_370;
   struct {
          TEXT  jrd_383 [32];	// RDB$DEPENDENT_NAME 
          SSHORT jrd_384;	// gds__utility 
          SSHORT jrd_385;	// RDB$DEPENDENT_TYPE 
   } jrd_382;
   struct {
          TEXT  jrd_379 [32];	// RDB$FIELD_NAME 
          TEXT  jrd_380 [32];	// RDB$DEPENDED_ON_NAME 
          SSHORT jrd_381;	// RDB$DEPENDED_ON_TYPE 
   } jrd_378;
/**************************************
 *
 ...
 **************************************/
	...
	if (field_name)
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_377, sizeof(jrd_377), true);
		gds__vtov ((const char*) field_name, (char*) jrd_378.jrd_379, 32);
		gds__vtov ((const char*) dpdo_name, (char*) jrd_378.jrd_380, 32);
		jrd_378.jrd_381 = dpdo_type;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 66, (UCHAR*) &jrd_378);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 36, (UCHAR*) &jrd_382);
		   if (!jrd_382.jrd_384) break;
			...
			if (!find_depend_in_dfw(tdbb, /*DEP.RDB$DEPENDENT_NAME*/
						      jrd_382.jrd_383, /*DEP.RDB$DEPENDENT_TYPE*/
  jrd_382.jrd_385,
									0, transaction))
			{
	...
	else
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_369, sizeof(jrd_369), true);
		gds__vtov ((const char*) dpdo_name, (char*) jrd_370.jrd_371, 32);
		jrd_370.jrd_372 = dpdo_type;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 34, (UCHAR*) &jrd_370);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 36, (UCHAR*) &jrd_373);
		   if (!jrd_373.jrd_375) break;
			...
			if (!find_depend_in_dfw(tdbb, /*DEP.RDB$DEPENDENT_NAME*/
						      jrd_373.jrd_374, /*DEP.RDB$DEPENDENT_TYPE*/
  jrd_373.jrd_376,
									0, transaction))
			{
				++dep_counts[/*DEP.RDB$DEPENDENT_TYPE*/
...
static void cleanup_index_creation(thread_db* tdbb, DeferredWork* work, jrd_tra* transaction)
{
   ...
   struct {
          SSHORT jrd_365;	// gds__null_flag 
          SSHORT jrd_366;	// RDB$INDEX_ID 
   } jrd_364;
   struct {
          TEXT  jrd_360 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_361;	// gds__utility 
          SSHORT jrd_362;	// gds__null_flag 
          SSHORT jrd_363;	// RDB$INDEX_ID 
   } jrd_359;
   struct {
	...
	/*FOR(REQUEST_HANDLE request TRANSACTION_HANDLE transaction) IDXN IN RDB$INDICES CROSS
		IREL IN RDB$RELATIONS OVER RDB$RELATION_NAME
			...
			AND IREL.RDB$VIEW_BLR MISSING*/
	{
	if (!request)
	   request = CMP_compile2 (tdbb, (UCHAR*) jrd_356, sizeof(jrd_356), true);
	gds__vtov ((const char*) work->dfw_name.c_str(), (char*) jrd_357.jrd_358, 32);
	EXE_start (tdbb, request, transaction);
	EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_357);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 38, (UCHAR*) &jrd_359);
	   if (!jrd_359.jrd_361) break; // views do not have indices
	{
		...
		if (relPages && relPages->rel_index_root)
		{
			     ...
			     jrd_359.jrd_362)
			{
				/*MODIFY IDXN USING*/
				{
					...
					jrd_359.jrd_362 = TRUE;
				/*END_MODIFY*/
				jrd_364.jrd_365 = jrd_359.jrd_362;
				jrd_364.jrd_366 = jrd_359.jrd_363;
				EXE_send (tdbb, request, 2, 4, (UCHAR*) &jrd_364);
				}
			}
		}
	}
	/*END_FOR*/
	   EXE_send (tdbb, request, 3, 2, (UCHAR*) &jrd_367);
	   }
	}
	...
	case 3:
		{
			...
			{
			if (!handle)
			   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_351, sizeof(jrd_351), true);
			gds__vtov ((const char*) work->dfw_name.c_str(), (char*) jrd_352.jrd_353, 32);
			EXE_start (tdbb, handle, dbb->dbb_sys_trans);
			EXE_send (tdbb, handle, 0, 32, (UCHAR*) &jrd_352);
			while (1)
			   {
			   EXE_receive (tdbb, handle, 1, 2, (UCHAR*) &jrd_354);
			   if (!jrd_354.jrd_355) break;
...
{
   struct {
          SSHORT jrd_348;	// gds__utility 
          SSHORT jrd_349;	// RDB$RELATION_ID 
          SSHORT jrd_350;	// RDB$RELATION_TYPE 
   } jrd_347;
   struct {
 ...
 **************************************/
	...
	if (have_gtt)
	{
		...
		if (is_create)
		{
			...
			EXE_start (tdbb, request, dbb->dbb_sys_trans);
			EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_345);
			while (1)
			   {
			   EXE_receive (tdbb, request, 1, 6, (UCHAR*) &jrd_347);
			   if (!jrd_347.jrd_348) break;
				gtt_preserve = (/*REL.RDB$RELATION_TYPE*/
						jrd_347.jrd_350 == rel_global_temp_preserve);
				relation = MET_lookup_relation_id(tdbb, /*REL.RDB$RELATION_ID*/
									jrd_347.jrd_349, false);
			/*END_FOR;*/
			   }
			}
    ...
    jrd_321.jrd_324 < 0.0)
		...
		try
		{
			...
			if (!(idx.idx_flags & idx_foreign)) {
				relationLock = DFW_protect_relation(tdbb, transaction, relation, 
								releaseRelationLock);
			}
			else
			{
				...
				if (relation->rel_id < partner_relation->rel_id) 
				{
					relationLock = DFW_protect_relation(tdbb, transaction, relation, 
									releaseRelationLock);
					partnerLock = DFW_protect_relation(tdbb, transaction, partner_relation, 
									releasePartnerLock);
				}
				else
				{
...
*/
			...
			if (partnerLock && releasePartnerLock) {
				DFW_release_protect_lock(tdbb, transaction, partnerLock);
			}
		}
...
Lock* DFW_protect_relation(thread_db* tdbb, jrd_tra* transaction, jrd_rel* relation, bool& releaseLock)
{
/**************************************
 *
...
void DFW_release_protect_lock(thread_db* tdbb, jrd_tra*	transaction, Lock* relLock)
{
