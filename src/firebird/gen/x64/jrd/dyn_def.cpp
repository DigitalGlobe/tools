...
static const UCHAR	jrd_725 [120] =
   {	// blr string 

4,2,4,0,12,0,41,3,0,32,0,41,3,0,32,0,9,0,7,0,7,0,7,0,7,0,7,0,
7,0,7,0,7,0,7,0,12,0,15,'K',29,0,0,2,1,41,0,4,0,3,0,24,0,1,0,
1,25,0,5,0,24,0,3,0,1,25,0,0,0,24,0,0,0,1,41,0,7,0,6,0,24,0,2,
0,1,41,0,1,0,8,0,24,0,7,0,1,41,0,2,0,9,0,24,0,8,0,1,41,0,11,0,
10,0,24,0,4,0,255,255,76
   };	// end of blr string 
...
{
   struct {
          SSHORT jrd_717;	// gds__utility 
          SSHORT jrd_718;	// RDB$COLLATION_ID 
   } jrd_716;
   struct {
          SSHORT jrd_715;	// RDB$CHARACTER_SET_ID 
   } jrd_714;
   struct {
          TEXT  jrd_723 [32];	// RDB$CHARACTER_SET_NAME 
          SSHORT jrd_724;	// gds__utility 
   } jrd_722;
   struct {
          SSHORT jrd_721;	// RDB$CHARACTER_SET_ID 
   } jrd_720;
   struct {
          TEXT  jrd_727 [32];	// RDB$COLLATION_NAME 
          TEXT  jrd_728 [32];	// RDB$BASE_COLLATION_NAME 
          bid  jrd_729;	// RDB$SPECIFIC_ATTRIBUTES 
          SSHORT jrd_730;	// gds__null_flag 
          SSHORT jrd_731;	// RDB$COLLATION_ID 
          SSHORT jrd_732;	// RDB$COLLATION_ATTRIBUTES 
          SSHORT jrd_733;	// gds__null_flag 
          SSHORT jrd_734;	// RDB$CHARACTER_SET_ID 
          SSHORT jrd_735;	// gds__null_flag 
          SSHORT jrd_736;	// gds__null_flag 
          SSHORT jrd_737;	// gds__null_flag 
          SSHORT jrd_738;	// RDB$SYSTEM_FLAG 
   } jrd_726;
/**************************************
 *
 ...
 **************************************/
	...
	try
	{
			...
			X IN RDB$COLLATIONS*/
		{
			...
			jrd_726.jrd_737 = FALSE;
			/*X.RDB$SPECIFIC_ATTRIBUTES.NULL*/
			jrd_726.jrd_736 = TRUE;
			/*X.RDB$BASE_COLLATION_NAME.NULL*/
			jrd_726.jrd_735 = TRUE;
			...
			while (**ptr != isc_dyn_end)
			{
				switch (*(*ptr)++)
				{
					case isc_dyn_coll_for_charset:
					{
						...
						jrd_726.jrd_733 = FALSE;
						/*X.RDB$CHARACTER_SET_ID*/
						jrd_726.jrd_734 = DYN_get_number(ptr);
						...
						{
						if (!request2)
						   request2 = CMP_compile2 (tdbb, (UCHAR*) jrd_719, sizeof(jrd_719), true);
						jrd_720.jrd_721 = jrd_726.jrd_734;
						EXE_start (tdbb, request2, gbl->gbl_transaction);
						EXE_send (tdbb, request2, 0, 2, (UCHAR*) &jrd_720);
						while (1)
						   {
						   EXE_receive (tdbb, request2, 1, 34, (UCHAR*) &jrd_722);
						   if (!jrd_722.jrd_724) break;
					...
					case isc_dyn_coll_from:
						...
						/*X.RDB$BASE_COLLATION_NAME.NULL*/
						jrd_726.jrd_735 = FALSE;
						strcpy(/*X.RDB$BASE_COLLATION_NAME*/
						       jrd_726.jrd_728, info.baseCollationName.c_str());
			...
			strcpy(/*X.RDB$COLLATION_NAME*/
			       jrd_726.jrd_727, collation_name.c_str());
			...
			jrd_726.jrd_732 = info.attributes;

			if (info.specificAttributes.getCount() != 0)
			{
				/*X.RDB$SPECIFIC_ATTRIBUTES.NULL*/
				jrd_726.jrd_736 = FALSE;
			...
			jrd_714.jrd_715 = jrd_726.jrd_734;
			EXE_start (tdbb, request2, dbb->dbb_sys_trans);
			EXE_send (tdbb, request2, 0, 2, (UCHAR*) &jrd_714);
			while (1)
			   {
			   EXE_receive (tdbb, request2, 1, 4, (UCHAR*) &jrd_716);
			   if (!jrd_716.jrd_717) break;

				if (!REQUEST(irq_l_colls))
					REQUEST(irq_l_colls) = request2;

				if (/*Y.RDB$COLLATION_ID*/
				    jrd_716.jrd_718 + 1 <= /*X.RDB$COLLATION_ID*/
	jrd_726.jrd_731)
				{
					/*X.RDB$COLLATION_ID.NULL*/
					jrd_726.jrd_730 = FALSE;
					EXE_unwind(tdbb, request2);
					break;
				}
				else
					/*X.RDB$COLLATION_ID*/
					jrd_726.jrd_731 = /*Y.RDB$COLLATION_ID*/
   jrd_716.jrd_718 - 1;
		...
		/*END_STORE;*/
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_725, sizeof(jrd_725), true);
		EXE_start (tdbb, request, gbl->gbl_transaction);
		EXE_send (tdbb, request, 0, 90, (UCHAR*) &jrd_726);
		}
