...
static const UCHAR	jrd_458 [79] =
   {	// blr string 

4,2,4,1,2,0,7,0,7,0,4,0,1,0,7,0,12,0,2,7,'C',1,'K',6,0,0,'G',
47,24,0,3,0,25,0,0,0,255,14,1,2,1,21,8,0,1,0,0,0,25,1,0,0,1,24,
0,6,0,25,1,1,0,255,14,1,1,21,8,0,0,0,0,0,25,1,0,0,255,255,76
   };	// end of blr string 
static const UCHAR	jrd_464 [148] =
   {	// blr string 
...
static const UCHAR	jrd_474 [134] =
   {	// blr string 
...
static const UCHAR	jrd_483 [163] =
   {	// blr string 
...
static const UCHAR	jrd_494 [149] =
   {	// blr string 
...
static const UCHAR	jrd_511 [118] =
   {	// blr string 
...
static const UCHAR	jrd_521 [122] =
   {	// blr string 
...
static const UCHAR	jrd_537 [255] =
   {	// blr string 
...
static const UCHAR	jrd_560 [182] =
   {	// blr string 
...
{
   struct {
          bid  jrd_564;	// RDB$VALIDATION_BLR 
          bid  jrd_565;	// RDB$DEFAULT_VALUE 
          SSHORT jrd_566;	// gds__utility 
          SSHORT jrd_567;	// gds__null_flag 
          SSHORT jrd_568;	// gds__null_flag 
          SSHORT jrd_569;	// gds__null_flag 
          SSHORT jrd_570;	// RDB$NULL_FLAG 
          SSHORT jrd_571;	// RDB$COLLATION_ID 
          SSHORT jrd_572;	// RDB$CHARACTER_SET_ID 
          SSHORT jrd_573;	// RDB$FIELD_SUB_TYPE 
          SSHORT jrd_574;	// RDB$FIELD_LENGTH 
          SSHORT jrd_575;	// RDB$FIELD_SCALE 
          SSHORT jrd_576;	// RDB$FIELD_TYPE 
   } jrd_563;
   struct {
          TEXT  jrd_562 [32];	// RDB$FIELD_NAME 
   } jrd_561;
/**************************************
 *
 ...
 **************************************/
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_560, sizeof(jrd_560), true);
	gds__vtov ((const char*) name.c_str(), (char*) jrd_561.jrd_562, 32);
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	EXE_send (tdbb, handle, 0, 32, (UCHAR*) &jrd_561);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 1, 38, (UCHAR*) &jrd_563);
	   if (!jrd_563.jrd_566) break;
								...
								jrd_563.jrd_571))
		{
			...
			if (fieldInfo)
			{
				fieldInfo->nullable = /*FLD.RDB$NULL_FLAG.NULL*/
						      jrd_563.jrd_569 || /*FLD.RDB$NULL_FLAG*/
    jrd_563.jrd_570 == 0;
...
{
   struct {
          bid  jrd_542;	// RDB$VALIDATION_BLR 
          bid  jrd_543;	// RDB$DEFAULT_VALUE 
          bid  jrd_544;	// RDB$DEFAULT_VALUE 
          TEXT  jrd_545 [32];	// RDB$FIELD_SOURCE 
          SSHORT jrd_546;	// gds__utility 
          SSHORT jrd_547;	// gds__null_flag 
          SSHORT jrd_548;	// gds__null_flag 
          SSHORT jrd_549;	// gds__null_flag 
          SSHORT jrd_550;	// gds__null_flag 
          SSHORT jrd_551;	// RDB$NULL_FLAG 
          SSHORT jrd_552;	// gds__null_flag 
          SSHORT jrd_553;	// RDB$NULL_FLAG 
          SSHORT jrd_554;	// RDB$COLLATION_ID 
          SSHORT jrd_555;	// RDB$CHARACTER_SET_ID 
          SSHORT jrd_556;	// RDB$FIELD_SUB_TYPE 
          SSHORT jrd_557;	// RDB$FIELD_LENGTH 
          SSHORT jrd_558;	// RDB$FIELD_SCALE 
          SSHORT jrd_559;	// RDB$FIELD_TYPE 
   } jrd_541;
   struct {
          TEXT  jrd_539 [32];	// RDB$FIELD_NAME 
          TEXT  jrd_540 [32];	// RDB$RELATION_NAME 
   } jrd_538;
/**************************************
 *
 ...
 **************************************/
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_537, sizeof(jrd_537), true);
	gds__vtov ((const char*) fieldName.c_str(), (char*) jrd_538.jrd_539, 32);
	gds__vtov ((const char*) relationName.c_str(), (char*) jrd_538.jrd_540, 32);
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	EXE_send (tdbb, handle, 0, 64, (UCHAR*) &jrd_538);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 1, 84, (UCHAR*) &jrd_541);
	   if (!jrd_541.jrd_546) break;
			...
			if (fieldInfo)
			{
				fieldInfo->nullable = /*RFL.RDB$NULL_FLAG.NULL*/
					...
					(/*FLD.RDB$NULL_FLAG.NULL*/
					 jrd_541.jrd_550 || /*FLD.RDB$NULL_FLAG*/
    jrd_541.jrd_551 == 0) : /*RFL.RDB$NULL_FLAG*/
	 jrd_541.jrd_553 == 0;
				...
				if (!/*FLD.RDB$VALIDATION_BLR.NULL*/
				     jrd_541.jrd_547)
					fieldInfo->validation = parse_field_blr(tdbb, &/*FLD.RDB$VALIDATION_BLR*/
										       jrd_541.jrd_542, /*RFL.RDB$FIELD_SOURCE*/
  jrd_541.jrd_545);
				else
					fieldInfo->validation = NULL;
			}
...
{
   struct {
          SSHORT jrd_520;	// gds__utility 
   } jrd_519;
   struct {
          SSHORT jrd_518;	// RDB$SHADOW_NUMBER 
   } jrd_517;
   struct {
          SSHORT jrd_515;	// gds__utility 
          SSHORT jrd_516;	// RDB$SHADOW_NUMBER 
   } jrd_514;
   struct {
          SSHORT jrd_513;	// RDB$SHADOW_NUMBER 
   } jrd_512;
   struct {
          SSHORT jrd_529;	// gds__utility 
   } jrd_528;
   struct {
          SSHORT jrd_527;	// gds__utility 
   } jrd_526;
   struct {
          TEXT  jrd_523 [256];	// RDB$FILE_NAME 
          SSHORT jrd_524;	// gds__utility 
          SSHORT jrd_525;	// RDB$SHADOW_NUMBER 
   } jrd_522;
   struct {
          SSHORT jrd_536;	// gds__utility 
   } jrd_535;
   struct {
...
/* Erase any secondary files of the primary database of the
   shadow being activated. */
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_530, sizeof(jrd_530), true);
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 0, 2, (UCHAR*) &jrd_531);
	   if (!jrd_531.jrd_532) break;
		/*ERASE X;*/
		EXE_send (tdbb, handle, 1, 2, (UCHAR*) &jrd_533);
	/*END_FOR;*/
	   EXE_send (tdbb, handle, 2, 2, (UCHAR*) &jrd_535);
	   }
	}
...
/* go through files looking for any that expand to the current database name */
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_521, sizeof(jrd_521), true);
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 0, 260, (UCHAR*) &jrd_522);
	   if (!jrd_522.jrd_524) break;

		PIO_expand(/*X.RDB$FILE_NAME*/
			   jrd_522.jrd_523, (USHORT)strlen(/*X.RDB$FILE_NAME*/
		 jrd_522.jrd_523),
		...
		if (!strcmp(expanded_name, dbb_file_name)) {
			...
			{
			if (!handle2)
			   handle2 = CMP_compile2 (tdbb, (UCHAR*) jrd_511, sizeof(jrd_511), true);
			jrd_512.jrd_513 = jrd_522.jrd_525;
			EXE_start (tdbb, handle2, dbb->dbb_sys_trans);
			EXE_send (tdbb, handle2, 0, 2, (UCHAR*) &jrd_512);
			while (1)
			   {
			   EXE_receive (tdbb, handle2, 1, 4, (UCHAR*) &jrd_514);
			   if (!jrd_514.jrd_515) break;
				/*MODIFY Y*/
				{
					...
					jrd_514.jrd_516 = 0;
				/*END_MODIFY;*/
				jrd_517.jrd_518 = jrd_514.jrd_516;
				EXE_send (tdbb, handle2, 2, 2, (UCHAR*) &jrd_517);
				}
			/*END_FOR;*/
			   EXE_send (tdbb, handle2, 3, 2, (UCHAR*) &jrd_519);
			   }
			}
			/*ERASE X;*/
			EXE_send (tdbb, handle, 1, 2, (UCHAR*) &jrd_526);
		}
	/*END_FOR;*/
	   EXE_send (tdbb, handle, 2, 2, (UCHAR*) &jrd_528);
	   }
	}
...
{
   struct {
          TEXT  jrd_470 [32];	// RDB$RELATION_NAME 
          TEXT  jrd_471 [32];	// RDB$TRIGGER_NAME 
          SSHORT jrd_472;	// gds__utility 
          SSHORT jrd_473;	// RDB$TRIGGER_TYPE 
   } jrd_469;
   struct {
          TEXT  jrd_466 [32];	// RDB$DEPENDED_ON_NAME 
          SSHORT jrd_467;	// RDB$DEPENDENT_TYPE 
          SSHORT jrd_468;	// RDB$DEPENDED_ON_TYPE 
   } jrd_465;
   struct {
          TEXT  jrd_480 [32];	// RDB$PROCEDURE_NAME 
          SSHORT jrd_481;	// gds__utility 
          SSHORT jrd_482;	// RDB$PROCEDURE_ID 
   } jrd_479;
   struct {
          TEXT  jrd_476 [32];	// RDB$DEPENDED_ON_NAME 
          SSHORT jrd_477;	// RDB$DEPENDENT_TYPE 
          SSHORT jrd_478;	// RDB$DEPENDED_ON_TYPE 
   } jrd_475;
   struct {
          TEXT  jrd_490 [32];	// RDB$RELATION_NAME 
          TEXT  jrd_491 [32];	// RDB$TRIGGER_NAME 
          SSHORT jrd_492;	// gds__utility 
          SSHORT jrd_493;	// RDB$TRIGGER_TYPE 
   } jrd_489;
   struct {
          TEXT  jrd_485 [32];	// RDB$FIELD_NAME 
          TEXT  jrd_486 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_487;	// RDB$DEPENDENT_TYPE 
          SSHORT jrd_488;	// RDB$DEPENDED_ON_TYPE 
   } jrd_484;
   struct {
          TEXT  jrd_501 [32];	// RDB$PROCEDURE_NAME 
          SSHORT jrd_502;	// gds__utility 
          SSHORT jrd_503;	// RDB$PROCEDURE_ID 
   } jrd_500;
   struct {
          TEXT  jrd_496 [32];	// RDB$FIELD_NAME 
          TEXT  jrd_497 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_498;	// RDB$DEPENDENT_TYPE 
          SSHORT jrd_499;	// RDB$DEPENDED_ON_TYPE 
   } jrd_495;
   struct {
          TEXT  jrd_508 [32];	// RDB$FIELD_NAME 
          TEXT  jrd_509 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_510;	// gds__utility 
   } jrd_507;
   struct {
          TEXT  jrd_506 [32];	// RDB$FIELD_SOURCE 
   } jrd_505;
/**************************************
 *
 ...
 **************************************/
	...
	{
	if (!request)
	   request = CMP_compile2 (tdbb, (UCHAR*) jrd_504, sizeof(jrd_504), true);
	gds__vtov ((const char*) field_source->dsc_address, (char*) jrd_505.jrd_506, 32);
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_505);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 66, (UCHAR*) &jrd_507);
	   if (!jrd_507.jrd_510) break;
		...
		if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
		{
			...
			gds__vtov ((const char*) jrd_507.jrd_508, (char*) jrd_495.jrd_496, 32);
			gds__vtov ((const char*) jrd_507.jrd_509, (char*) jrd_495.jrd_497, 32);
			jrd_495.jrd_498 = obj_procedure;
			jrd_495.jrd_499 = obj_relation;
			EXE_start (tdbb, request2, dbb->dbb_sys_trans);
			EXE_send (tdbb, request2, 0, 68, (UCHAR*) &jrd_495);
			while (1)
			   {
			   EXE_receive (tdbb, request2, 1, 36, (UCHAR*) &jrd_500);
			   if (!jrd_500.jrd_502) break;
			...
			{
			if (!request2)
			   request2 = CMP_compile2 (tdbb, (UCHAR*) jrd_483, sizeof(jrd_483), true);
			gds__vtov ((const char*) jrd_507.jrd_508, (char*) jrd_484.jrd_485, 32);
			gds__vtov ((const char*) jrd_507.jrd_509, (char*) jrd_484.jrd_486, 32);
			jrd_484.jrd_487 = obj_trigger;
			jrd_484.jrd_488 = obj_relation;
			EXE_start (tdbb, request2, dbb->dbb_sys_trans);
			EXE_send (tdbb, request2, 0, 68, (UCHAR*) &jrd_484);
			while (1)
			   {
			   EXE_receive (tdbb, request2, 1, 68, (UCHAR*) &jrd_489);
			   if (!jrd_489.jrd_492) break;
				...
				DFW_post_work_arg(transaction, dw2, NULL, /*TRG.RDB$TRIGGER_TYPE*/
									  jrd_489.jrd_493, dfw_arg_trg_type);
	...
	if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
	{
		...
		gds__vtov ((const char*) field_source->dsc_address, (char*) jrd_475.jrd_476, 32);
		jrd_475.jrd_477 = obj_procedure;
		jrd_475.jrd_478 = obj_field;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 36, (UCHAR*) &jrd_475);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 36, (UCHAR*) &jrd_479);
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_464, sizeof(jrd_464), true);
		gds__vtov ((const char*) field_source->dsc_address, (char*) jrd_465.jrd_466, 32);
		jrd_465.jrd_467 = obj_trigger;
		jrd_465.jrd_468 = obj_field;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 36, (UCHAR*) &jrd_465);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 68, (UCHAR*) &jrd_469);
		   if (!jrd_469.jrd_472) break;
			...
			DeferredWork* dw2 = DFW_post_work(transaction, dfw_modify_trigger, &desc, 0);
			DFW_post_work_arg(transaction, dw2, NULL, /*TRG.RDB$TRIGGER_TYPE*/
								  jrd_469.jrd_473, dfw_arg_trg_type);
...
Format* MET_current(thread_db* tdbb, jrd_rel* relation)
{
   struct {
          ...
          SSHORT jrd_463;	// RDB$FORMAT 
   } jrd_461;
   struct {
          SSHORT jrd_460;	// RDB$RELATION_ID 
   } jrd_459;
/**************************************
 *
 ...
 **************************************/
	...
	if (!(relation->rel_flags & REL_scanned))
	{
		...
		jrd_459.jrd_460 = relation->rel_id;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 2, (UCHAR*) &jrd_459);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 4, (UCHAR*) &jrd_461);
	...
	while (1)
	   ...
	   if (!jrd_394.jrd_396) break;
		...
		MET_load_trigger(tdbb, NULL, /*TRG.RDB$TRIGGER_NAME*/
					     jrd_394.jrd_395, &dbb->dbb_triggers[type]);
