...
static const UCHAR	jrd_141 [143] =
   {	// blr string 
...
15,3,0,11,0,'P','R','I','M','A','R','Y',32,'K','E','Y','F',1,
'H',24,1,2,0,255,14,1,2,1,24,1,1,0,25,1,0,0,1,21,8,0,1,0,0,0,
...
static const UCHAR	jrd_210 [180] =
   {	// blr string 
...
2,0,0,21,15,3,0,22,0,'R','D','B','$','C','H','A','R','A','C',
'T','E','R','_','S','E','T','_','N','A','M','E',47,24,1,3,0,24,
0,0,0,255,14,1,2,1,21,8,0,1,0,0,0,25,1,0,0,1,24,1,8,0,41,1,2,
0,1,0,1,24,0,1,0,25,1,3,0,1,24,0,2,0,25,1,4,0,255,14,1,1,21,8,
...
static const UCHAR	jrd_239 [84] =
   {	// blr string 

4,2,4,1,1,0,7,0,4,0,1,0,41,3,0,32,0,12,0,2,7,'C',1,'K',2,0,0,
'G',58,47,24,0,0,0,25,0,0,0,46,24,0,15,0,21,8,0,1,0,0,0,255,14,
1,2,1,21,8,0,1,0,0,0,25,1,0,0,255,14,1,1,21,8,0,0,0,0,0,25,1,
0,0,255,255,76
   };	// end of blr string 
static const UCHAR	jrd_244 [84] =
   {	// blr string 

4,2,4,1,1,0,7,0,4,0,1,0,41,3,0,32,0,12,0,2,7,'C',1,'K',6,0,0,
'G',58,47,24,0,8,0,25,0,0,0,46,24,0,4,0,21,8,0,1,0,0,0,255,14,
1,2,1,21,8,0,1,0,0,0,25,1,0,0,255,14,1,1,21,8,0,0,0,0,0,25,1,
0,0,255,255,76
   };	// end of blr string 
...
namespace {
	...
	bool isSystemRelation(thread_db* tdbb, dsql_dbb* dbb, jrd_tra* transaction, const char* relName)
	{
	   ...
	   struct {
	          TEXT  jrd_246 [32];	// RDB$RELATION_NAME 
	   } jrd_245;
		bool rc = false;
		...
		{
		if (!handle)
		   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_244, sizeof(jrd_244), true);
		gds__vtov ((const char*) relName, (char*) jrd_245.jrd_246, 32);
		EXE_start (tdbb, handle, transaction);
		EXE_send (tdbb, handle, 0, 32, (UCHAR*) &jrd_245);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 2, (UCHAR*) &jrd_247);
		   if (!jrd_247.jrd_248) break;
		{
			rc = true;
		}
		/*END_FOR*/
	...
	bool isSystemDomain(thread_db* tdbb, dsql_dbb* dbb, jrd_tra* transaction, const char* fldName)
	{
	   ...
	   struct {
	          TEXT  jrd_241 [32];	// RDB$FIELD_NAME 
	   } jrd_240;
		bool rc = false;
		...
		{
		if (!handle)
		   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_239, sizeof(jrd_239), true);
		gds__vtov ((const char*) fldName, (char*) jrd_240.jrd_241, 32);
		EXE_start (tdbb, handle, transaction);
		EXE_send (tdbb, handle, 0, 32, (UCHAR*) &jrd_240);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 2, (UCHAR*) &jrd_242);
		   if (!jrd_242.jrd_243) break;
		{
			rc = true;
		}
		/*END_FOR*/
