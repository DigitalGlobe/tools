...
Scene_Context_Jonswap::Scene_Context_Jonswap(size_t index_size_x,
					     size_t index_size_y,
  ...
  : Scene_Context(index_size_x,
		  ...
		  index_wind_angle,
		  index_surface_alpha),
    fetch(new class Config_Array<>(Fetch, Fetch_s, index_fetch))
{
}


Scene_Context_Jonswap::~Scene_Context_Jonswap(void)
{
  delete this->fetch;
}
...
/*  fetch  */
bool
Scene_Context_Jonswap::decrease_fetch(void)
{
...
bool
Scene_Context_Jonswap::increase_fetch(void)
{
...
float
Scene_Context_Jonswap::get_fetch(void) const
{
