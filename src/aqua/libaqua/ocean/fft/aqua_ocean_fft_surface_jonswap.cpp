...
#include "aqua_ocean_fft_surface_jonswap.h"
...
Surface_Jonswap::Surface_Jonswap(enum type fft_type,
  ...
  : Surface(spectrum::directional_create_jonswap_longuet(wind_speed, fetch),
	    rng,
	    fft_type,
	    size_x,
	    size_y,
...
/*  virtual  */
Surface_Jonswap::~Surface_Jonswap(void)
{
}
...
void
Surface_Jonswap::set_fetch(float fetch)
{
//   dynamic_cast<class aqua::spectrum::Jonswap * const>(this->spectrum)
//     ->set_fetch(fetch);

//   this->wave_set->set_spectrum(*this->spectrum);
...
float
Surface_Jonswap::get_fetch(void) const
{
//   return
//     dynamic_cast<class aqua::spectrum::Jonswap * const>(this->spectrum)->get_fetch();
  return 0.0;
}
