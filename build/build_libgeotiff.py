...
class Program :
    ...
    # public methods
        ...
        def main(self) :
            ...
            proj4Lib = pathFinder.path(sdkOutDir, Program._PROJ4_LIB + \
                ( '' if ( buildSettings.ReleaseSpecified() )  else Program._DEBUG_SUFFIX) + ".lib")
            tiffLib = pathFinder.path(sdkOutDir, Program._TIFF_LIB + \
                ( '' if ( buildSettings.ReleaseSpecified() )  else Program._DEBUG_SUFFIX) + ".lib")

            proj4Include = pathFinder.path(buildPathName, Program._PROJ4_INCLUDE)
            tiffInclude = pathFinder.path(buildPathName, Program._TIFF_INCLUDE)
            ...
            systemManager.distributeFiles(pathFinder.path( buildPathName, Program._PATH_NAME_INCLUDE),              \
                                          ...
                                          '*.inc',                                                                 \
                                          True, False)
            systemManager.distributeFiles(pathFinder.path( buildPathName, Program._PATH_NAME_XTIFF_INCLUDE),              \
                                          pathFinder.path( buildPathName, Program._PATH_NAME_DISTRIBUTION_INCLUDE), \
                                          '*.h',                                                                 \
                                          True, False)
