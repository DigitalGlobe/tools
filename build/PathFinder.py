...
class PathFinder :
    ...
    # public methods
        ...
        # Returns :
        #     the name of the Visual Studio bin path
        def getVisualStudioIDEPathName( self         , \
                                        x64Specified ) :
        
            pathName = os.path.join( self.getVisualStudioPathName(x64Specified), PathFinder._PATH_NAME_IDE)
            if ( not os.path.exists(pathName) ) :
                raise Exception(PathFinder._ERROR_PATH_BIN)
            return pathName
        #----------------------------------------------------------------------
