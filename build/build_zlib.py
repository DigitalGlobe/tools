...
class Program :
    ...
    # constants
        ...
        _PATH_NAME_DISTRIBUTION_INCLUDE = '..\\..\\include\\zlib'
        #----------------------------------------------------------------------
    ...
    # public methods
        ...
        def main(self) :
            ...
            # run Nmake
            nmakeCommandLine = 'nmake -f win32/Makefile.msc'
            if ( buildSettings.X64Specified() ) :            
                nmakeCommandLine += ' MACHINE="/MACHINE:X64\"'
                nmakeCommandLine += ' AS=ml64 OBJA="inffasx64.obj gvmat64.obj inffas8664.obj"'
                nmakeCommandLine += ' LOC="-DASMV -DASMINF /I."'
            else :
                nmakeCommandLine += ' MACHINE="/MACHINE:X86"'
                nmakeCommandLine += ' OBJA="inffas32.obj match686.obj"'            
                nmakeCommandLine += ' LOC="-DASMV -DASMINF"'

            if ( buildSettings.ReleaseSpecified() ) :
                nmakeCommandLine += ' LINKOPT=/DEBUG:NONE OPTFLAGS=-MD'
            else :
                nmakeCommandLine += ' LINKOPT=/DEBUG:FULL OPTFLAGS="-Zi -MDd"'

            nmakeCommandLine += ' STATICLIB=' + staticLibName + ' SHAREDLIB=' + dllName + ' IMPLIB=' + libName
            ...
            systemManager.copyFile( os.path.join( buildPathName, dllName ) , \
                                    os.path.join( sdkOutDir , dllName) )
            systemManager.copyFile( os.path.join( buildPathName, staticLibName ) , \
                                    os.path.join( sdkOutDir , staticLibName) )
            if not buildSettings.ReleaseSpecified():
                systemManager.copyFile( os.path.join( buildPathName, pdbName ) , \
