...
class Program:
    ...
    _PATH_NAME_INCLUDE = "include\\zlib"
    # ----------------------------------------------------------------------
    ...
    def main(self):
        ...
        cmakeCommandLine = (
            ...
            + f"-DZLIB_BUILD_EXAMPLES=OFF "
            + f"-DCMAKE_INSTALL_PREFIX={os.path.join(cmakeBuildPath, "install")} "
            + f"{buildSourceName}"
        )
        ...
        systemManager.copyFile(
            os.path.join(
                srcLibPath,
                f"{Program._LIBNAME}static{"" if (buildSettings.ReleaseSpecified()) else "d"}.lib",
            ),
        ...
        systemManager.copyFile(
            os.path.join(
                srcBinPath,
                f"{Program._LIBNAME}{"" if (buildSettings.ReleaseSpecified()) else "d"}1.dll",
            ),
