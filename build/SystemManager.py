...
class SystemManager:
    ...
    _VALUE_INCLUDE_X64 = (
        f"{_pf.getMSVCIncludePathName(True)};"
        + f"{_pf.getATLIncludePathName(True)};"
        + f"{_pf.getWindowsSdkUCrtIncludePathName(True)};"
        + f"{_pf.getNetFxSdkIncludePathName(True)};"
        + f"{_pf.getWindowsSdkSharedIncludePathName(True)};"
        + f"{_pf.getWindowsSdkUMIncludePathName(True)};"
        + f"{_pf.getWindowsSdkWinRTIncludePathName(True)}"
    )
    # ----------------------------------------------------------------------
    ...
    _VALUE_INCLUDE_X86 = (
        f"{_pf.getMSVCIncludePathName(True)};"
        + f"{_pf.getATLIncludePathName(True)};"
        + f"{_pf.getWindowsSdkUCrtIncludePathName(False)};"
        + f"{_pf.getNetFxSdkIncludePathName(False)};"
        + f"{_pf.getWindowsSdkSharedIncludePathName(False)};"
        + f"{_pf.getWindowsSdkUMIncludePathName(False)};"
        + f"{_pf.getWindowsSdkWinRTIncludePathName(False)}"
    )
    # ----------------------------------------------------------------------
    ...
    _VALUE_LIBRARY_X64 = (
        f"{_pf.getMSVCLibraryPathName(True)};"
        + f"{_pf.getATLLibraryPathName(True)};"
        + f"{_pf.getWindowsSdkUCrtLibraryPathName(True)};"
        + f"{_pf.getNetFxSdkLibraryPathName(True)};"
        + f"{_pf.getWindowsSdkUMLibraryPathName(True)};"
    )
    ...
    _VALUE_LIBRARY_X86 = (
        f"{_pf.getMSVCLibraryPathName(True)};"
        + f"{_pf.getATLLibraryPathName(True)};"
        + f"{_pf.getWindowsSdkUCrtLibraryPathName(False)};"
        + f"{_pf.getNetFxSdkLibraryPathName(False)};"
        + f"{_pf.getWindowsSdkUMLibraryPathName(False)};"
    )
    ...
    def appendToIncludeEnvironmentVariable(self, pathName):

        self._appendToIncludeEnvironmentVariable(
            SystemManager._ENVIRONMENT_VARIABLE_NAME_INCLUDE, pathName
        )
    ...
    #     self     : this manager
    #     pathName : the name of the path to append
    def appendToPathEnvironmentVariable(self, pathName, prepend=False):

        self._appendToPathEnvironmentVariable(
            SystemManager._ENVIRONMENT_VARIABLE_NAME_PATH, pathName, prepend
        )
    ...
    def changeDirectory(self, pathName):
    ...
    def copyDirectory(self, sourcePathName, targetPathName):

        print('Directory "%s" -> "%s".' % (sourcePathName, targetPathName))
    ...
    def copyFile(self, sourceFileName, targetFileName):

        print('File "%s" -> "%s".' % (sourceFileName, targetFileName))
    ...
    def execute(self, commandLine):
        ...
        childProcess = subprocess.Popen(commandLine, shell=True)
    ...
    def getCurrentRelativePathName(self, pathName):

        return os.path.normpath(os.path.join(self.getCurrentPathName(), pathName))
    ...
    def getDebugFileName(self, fileName, dConsidered=False):
        ...
        if dConsidered and (
            ...
            or fileNameWithoutExtension.endswith("D")
        ):

            fileNameWithoutExtension = fileNameWithoutExtension[
                0 : (len(fileNameWithoutExtension) - 1)
            ]

        return fileNameWithoutExtension + SystemManager._DEBUG_SUFFIX + extension
    ...
    def getVisualStudioPath(self):
        ...
        value = value.replace(
            SystemManager._ENVIRONMENT_VARIABLE_PROGRAM_FILES_X86,
    ...
    def getIncludeEnvironmentVariableValue(self):
        ...
        return self._getEnvironmentVariableName(
            SystemManager._ENVIRONMENT_VARIABLE_NAME_INCLUDE
        )
        # ----------------------------------------------------------------------
    ...
    def getPathEnvironmentVariableValue(self):
        ...
        return self._getEnvironmentVariableName(
            SystemManager._ENVIRONMENT_VARIABLE_NAME_PATH
        )
    ...
    def getPathEnvironmentVariableValue(self):
        return self._getEnvironmentVariableName(
            SystemManager._ENVIRONMENT_VARIABLE_MSBUILD
        )
    ...
    def getProgramFilesPathName(self, x64Specified):

        return (
            self._getEnvironmentVariableName(
                SystemManager._ENVIRONMENT_VARIABLE_PROGRAM_FILES_X64
            )
            if (x64Specified)
            else self._getEnvironmentVariableName(
                SystemManager._ENVIRONMENT_VARIABLE_PROGRAM_FILES_X86
            )
        )
    ...
    def initializeIncludeEnvironmentVariable(self, x64Specified, additionalPath=None):

        value = (
            SystemManager._VALUE_INCLUDE_X64
            if (x64Specified)
    ...
    #                    <code>false</code>, 32-bit is specified
    #     additionalPath: optional additional include path to add at the  start of the includes
    def initializeLibraryEnvironmentVariable(self, x64Specified, additionalPath=None):

        value = (
            SystemManager._VALUE_LIBRARY_X64
            if (x64Specified)
    ...
    def removeDirectory(self, pathName):
    ...
    def _appendToIncludeEnvironmentVariable(self, environmentVariableName, pathName):
    ...
    #                               to which to append
    #     pathName                : the name of the path to append
    def _appendToPathEnvironmentVariable( self, environmentVariableName, pathName, prepend=False):

        if environmentVariableName in os.environ:
            if prepend:
                os.environ[environmentVariableName] = (
                    pathName
                    + SystemManager._VALUE_SEPARATOR
                    + os.environ[environmentVariableName]
                )
            else:
                if not os.environ[environmentVariableName].endswith( SystemManager._VALUE_SEPARATOR ):
    ...
    #     string if
    def _getEnvironmentVariableName(self, environmentVariableName):
